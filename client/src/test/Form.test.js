...
import CharacterCreate from "../components/Create";

describe("Form  Test", () => {
  let wrapper;
  beforeEach(() => {
    wrapper = mount(<CharacterCreate />);
  });
  it('El form debe tener un label que diga: "Breed"', () => {
    const { container } = render(<CharacterCreate />);
    const element = container.querySelectorAll("label")[0];
    expect(element.innerHTML).toBe("Breed");
  });
